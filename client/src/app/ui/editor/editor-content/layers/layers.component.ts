...
import { LayersFacadeService } from './layers.facade.service';
...
export class LayersComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit(): void {
    ...
    this.layersFacadeService.init();
    this.layersFacadeService.canvasDisplayRatio.subscribe((value) => {
      this.updateCursorRadius();
    });
    this.layersFacadeService.brushWidthChanged.subscribe(() => {
      this.updateCursorRadius();
    });
    this.layersFacadeService.selectedTool.subscribe((value) =>
      this.updateMouseCursor(value)
    );
    /*  */
  }
  ...
  showMouseCursor(): void {
    if (this.layersFacadeService.mousePositionInDisplayCoordinates) {
      ...
      this.setCursorPosition(
        this.layersFacadeService.mousePositionInDisplayCoordinates
      );
    }
  }
  ...
  onMouseIn(event: MouseEvent): void {
    const selectedToolName = this.layersFacadeService.selectedTool.getValue()
      .name;
    this.layersFacadeService.mousePositionInDisplayCoordinates = this.getPoint(
      new Point(event.clientX, event.clientY)
    );
    if (
      selectedToolName === TOOL_NAMES.BRUSH ||
      selectedToolName === TOOL_NAMES.ERASER
    ) {
      this.showMouseCursor();
    }
  }
  ...
  onTouchStart(event: TouchEvent): void {
    ...
    const selectedToolName = this.layersFacadeService.selectedTool.getValue()
      .name;
    this.layersFacadeService.mousePositionInDisplayCoordinates = this.getPoint(
      new Point(touch.clientX, touch.clientY)
    );
    if (
      selectedToolName === TOOL_NAMES.BRUSH ||
      selectedToolName === TOOL_NAMES.ERASER
  ...
  cursorMoveEvent(clientPoint: Point): void {
    ...
    this.layersFacadeService.mousePositionInDisplayCoordinates = new Point(
      point.x,
      point.y
    );
  }
  ...
  updateCursorRadius(): void {
    const r =
      (this.layersFacadeService.brushWidth / 2) *
      this.layersFacadeService.canvasDisplayRatio.getValue();
    if (r) {
  ...
  setDashStrokePosition(point: Point): void {
    this.dashStroke.nativeElement.setAttribute(
      'x1',
      this.layersFacadeService.lastPoint.x.toString()
    );
    this.dashStroke.nativeElement.setAttribute(
      'y1',
      this.layersFacadeService.lastPoint.y.toString()
    );
  ...
  getPoint(clientPoint: Point): Point {
    const x =
      this.layersFacadeService.scaleX === 1
        ? Math.ceil(
            clientPoint.x -
