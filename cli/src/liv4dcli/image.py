...
@image.command(
    name="create", help="Creates an image in the database from a file (required), and a revision (optional)."
)
...
@click.option(
    "--baseRevision",
    "base_revision",
    help="A base revision that will have to be corrected later",
    default="<?xml?><svg></svg>",
)
@click.option(
    "--finalRevision", "final_revision", help="The finalized version of a revision after correction", default=""
)
...
def _update(image_id, image_file, eye, hospital, patient, visit, code, image_type_id, base_revision, final_revision):
    update(
        image_id, image_file, eye, hospital, patient, visit, code, image_type_id, base_revision, final_revision, True
    )
...
def update(
    ...
    response = utils.send_file(
        "PUT", "/api/images/{}".format(image_id), ntpath.basename(image_file), image_data, "image", payload
    )
...
def update_base_revision(image_id, svg, display=False):
...
def update_biomarkers(image_id, biomarkers, display=False):
    svg_tree = ET.fromstring(get_base_revision(image_id))

    for biomarker, png in biomarkers.items():
        biomarker_node = [_ for _ in svg_tree.iter() if _.get("id") == biomarker]
        if len(biomarker_node) == 0:
            print(f"Could not find biomarker {biomarker} for image {image_id}")
            continue
        biomarker_node = biomarker_node[0]

        if isinstance(png, np.ndarray):
            png = utils.encode_png(png, color=biomarker_node.get("color"))
        if isinstance(png, bytes):
            png = png.decode("ascii")
        biomarker_node.set("{http://www.w3.org/1999/xlink}href", png)

    xml_header = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'
    svg = xml_header.encode(encoding="utf-8") + ET.tostring(svg_tree, encoding="utf-8")
    return update_base_revision(image_id=image_id, svg=svg.decode("utf-8"), display=display)
